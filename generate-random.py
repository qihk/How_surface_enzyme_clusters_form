...
AtomTypes = 5
Nsurface = 2700
Number_Type2 = int(Nsurface * 0.04)
...
def Choose_Type2(atoms, Number_Type2):
    max_index_for_selection = Nsurface

    if max_index_for_selection < Number_Type2:
        raise ValueError("no enough atoms")

    selected_indices = random.sample(range(max_index_for_selection), Number_Type2)
...
def Write_LAMMPS_Data(filename, atoms, atom_types):
    ...
    print(f"LAMMPS data written to {filename}, generate {Number_Type2} type2 atoms ===> random")
